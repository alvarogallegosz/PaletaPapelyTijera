...
def render_modulo_autenticacion():
    # Inicializar estados de la pantalla si no existen[cite: 8]
    if "auth_vista" not in st.session_state:
        st.session_state["auth_vista"] = "login"  # Opciones: login, registro, recuperar, verificar[cite: 8]
                    ...
                    try:
                        ...
                        if res.data:
                            ...
                            if not user_db.get("verificado", False):
                                ...
                                st.session_state["usuario_rol"] = user_db["rol"]  # Campo exacto: 'rol'[cite: 8]
    ...
    # --- PANTALLA 4: RECUPERACIÓN DE CONTRASEÑA ---
        ...
        if st.button("Generar Código Temporal", type="primary"):
            if email_rec:
                try:
    ...
    # --- PANTALLA 5: ESTABLECER NUEVA CONTRASEÑA ---
        ...
        if st.button("Actualizar Acceso", type="primary"):
